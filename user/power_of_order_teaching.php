...
<main class="main_content">
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				<h4 align="center"class="fw-bold">WELCOME TO THE TEACHING ON POWER OF ORDER WITH PASTOR TIBI PETERS</h4>
				<p class="lead">
					Welcome to the teaching and preaching of Pastor Tibi Peters on power of order as classics. You could get unlimited daily devotional, teachings on prayers that could help increasing your spiritual life and understanding on spiritual things and happenings as a believer in Christ Jesus. <br>
		...
		<div class="row">
			<?php if ($audio->num_rows>0): ?>
				<?php while($audio_row=$audio->fetch_object()): ?>
					<?php if ($audio_row->category=="classics" && $audio_row->sub_category=="the power of order"): ?>
		...
		<div class="row">
			<?php if ($video->num_rows>0): ?>
				<?php while($video_row=$video->fetch_object()): ?>
					<?php if ($video_row->category=="classics" && $video_row->sub_category=="the power of order"): ?>
		...
		<div class="row">
			<?php if ($pdf->num_rows>0): ?>
				<?php while($pdf_row=$pdf->fetch_object()): ?>
					<?php if ($pdf_row->category=="classics" && $pdf_row->sub_category=="the power of order"): ?>	
						<div class="col-md-4">
							<div class="card" style="width: 20rem; height:100%">
