<?php  
	...
	if (isset($_POST['upload_video_files'])) {
		...
		$category=post_clean("video_category",$db);
		$video=post_clean("video",$db);
		...
		$check_file=$db->query("SELECT * FROM video_media WHERE filename='$filename' ");
		if ($check_file->num_rows>0) {
			$status=0; $msg.="media filename already exist";
		}
		if ($status==1) {
			//uploading image file
			$image_upload="../art/video/".$_FILES['image']['name'];
			//uploading audio file
			$video_upload="../media/video/".$_FILES['video_content']['name'];
			move_uploaded_file($_FILES['video_content']['tmp_name'], $video_upload);
			move_uploaded_file($_FILES['image']['tmp_name'], $image_upload);
			$db->query("INSERT INTO video_media(title,filename,metadata,description,tags,publication_date,upload_date,file_type,category,sub_category,file,image)VALUES('$title','$filename','$meta_data','$description','$tags','$publication_date','$upload_date','$video','$category','$sub_category','$video_upload','$image_upload')");
			$_SESSION['msg']="".ucwords($file_type)."the $video media file $filename uploaded successfully";
			$_SESSION['color']="success";
			header("location:../admin/media_video_upload.php");
		}else{
		$_SESSION['msg']=$msg;
